...
void BasicDemo::clientMoveAndDisplay() //EN EL BUCLE
{
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); 

	//simple dynamics world doesn't handle fixed-time-stepping
	float ms = getDeltaTimeMicroseconds();
	std::cout << body->getWorldTransform().getOrigin().getZ();
	///step the simulation
	if (m_dynamicsWorld)
	{
...
//	btCollisionShape* groundShape = new btStaticPlaneShape(btVector3(0,1,0),50);
...

	{
		...
		startTransform.setOrigin(SCALING*btVector3(
							btScalar(2.0 + start_x),
							btScalar(20+2.0 + start_y),
							btScalar(2.0 + start_z)));
...
//
//	//add the body to the dynamics world
//	this->getDynamicsWorld()->addRigidBody(groundBody);
//}
